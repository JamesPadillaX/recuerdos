...
export const CollagePage: React.FC<CollagePageProps> = ({ onNext, onBack }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F3EDE2] relative overflow-hidden flex flex-col items-center justify-start pt-6 pb-12 px-2 selection:bg-[#722F37] selection:text-white">
      ...
      {/* Header controls */}
      <div className="w-full flex justify-start px-4 sm:px-8 mb-2 z-30">
        <button
          onClick={onBack}
      ...
      {/* Collage Area: 2 columnas en móvil, varias en PC */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-8 w-full max-w-6xl z-10 my-4 sm:my-8 px-2">
        {photos.map((photo, index) => {
          ...
          return (
            <motion.div
              ...
              className="relative bg-[#FDFBF7] p-2 pb-8 sm:p-3 sm:pb-12 shadow-lg hover:shadow-2xl rounded-sm border border-[#EBE3D5] cursor-pointer w-full"
              style={{ zIndex: index }}
            >
              <img 
                src={photo.url} 
